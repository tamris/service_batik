...
def send_email_otp(email, otp):
    ...
    msg = Message(subject=subject,
                  sender=(current_app.config['MAIL_FROM_NAME'], current_app.config['MAIL_USERNAME']),
                  recipients=[email])
...
def send_reset_password_email(email, otp):
    subject = "Reset Kata Sandi Sibatikgal"
    ...
    <body style="font-family: 'Arial', sans-serif; color: #3a2718; background: #fdf7f0; padding: 20px; margin: 0;">
        <div style="max-width: 600px; margin: 20px auto; padding: 20px; background-color: #ffffff; border-radius: 12px; box-shadow: 0px 4px 12px rgba(0, 0, 0, 0.15);">
            <header style="text-align: center; padding: 15px 20px; background: #c14762; border-radius: 10px 10px 0 0; color: #ffffff;">
                <h1 style="margin: 0; font-size: 24px; font-weight: 700; letter-spacing: 1px;">Reset Kata Sandi</h1>
            </header>
            <section style="padding: 30px; text-align: center; color: #6b4e3d;">
                <p style="margin: 0; font-size: 16px; font-weight: 500;">Halo,</p>
                <p style="font-size: 16px; margin-top: 10px;">Kami menerima permintaan untuk mengatur ulang kata sandi akun <strong>Sibatikgal</strong> Anda. Silakan gunakan kode berikut untuk melanjutkan:</p>
                <div style="margin: 20px auto; padding: 15px 20px; font-size: 36px; font-weight: bold; color: #9b3e4d; background-color: #f9e0e6; border: 2px solid #c14762; border-radius: 8px; display: inline-block;">
                    {{ otp }}
                </div>
                <p style="margin-top: 20px; font-size: 14px; color: #6b4e3d;">Kode ini akan kadaluarsa dalam <strong>5 menit</strong>. Jika Anda tidak meminta reset sandi, abaikan email ini dan pastikan akun Anda tetap aman.</p>
            </section>
            <footer style="background: #f7f0eb; text-align: center; padding: 15px; border-top: 1px solid #dbcbb1; border-radius: 0 0 10px 10px;">
                <p style="font-size: 14px; color: #946644;">
